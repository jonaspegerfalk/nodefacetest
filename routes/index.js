...
var GIFEncoder = require('gifencoder'); // Handle POST request 

router.post('/', function (req, res, next) {     
    var imgSrc = req.file ? req.file.path : '';   
    
    // Have to add extension due to error in face.js
    // https://github.com/felixrieseberg/project-oxford/issues/14
    //var imgSrcWithExtension = imgSrc+'.jpg';
    //fs.rename(imgSrc, imgSrcWithExtension);
    
    var imgSrcWithExtension = 'http://steezo.com/wp-content/uploads/2012/12/man-in-suit.jpg'
    
    console.log('imgSrc',imgSrcWithExtension)  
    Promise.resolve(imgSrcWithExtension)         
        .then(function detectFace(image) {
           var client = new oxford.Client(process.env.OXFORD_API);
    return client.face.detect({url: image, analyzesAge: true});
})    
...
}, function(a,b){console.log('detect',a,b);})    
